...
export function gateDecision(toolName, toolInput) {
  ...
  if (name === 'Bash') {
    // Strip literal -d/--data payloads and known prose heredoc bodies on the
    // WHOLE command BEFORE splitting. A payload is data, not an invocation; and
    // since splitSegments is NOT quote-aware, a shell separator (; && | &) INSIDE
    // a dispatch body would otherwise orphan a fragment that false-matches.
    // Stripping first blanks the body (incl. any separators in it), so the
    // URL/method args still match but the body text never does. A separator
    // OUTSIDE the payload still splits, so `curl -d '' x ; crontab -r` is still
    // caught.
    const safeCommand = stripKnownProseHeredocs(stripDataPayloads(String(toolInput?.command ?? '')))
